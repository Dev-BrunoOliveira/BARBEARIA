...
function BookingForm({ horario, fechar, aoConfirmar }) {
  ...
  const handleCelularChange = (e) => {
    ...
    if (value.length > 12) value = value.slice(0, 12); // Limita aos 12 dígitos solicitados
    
    // Aplica a máscara visual: 000 000000000
  ...
  const handleSubmit = (e) => {
    ...
    // Validação de Nome Completo (mínimo duas palavras)
    if (nome.trim().split(" ").length < 2) {
      alert("Por favor, insira seu nome completo (nome e sobrenome).");
      return;
    }
    
    // Validação de Telefone (exatamente 12 dígitos: 3 DDD + 9 número)
      ...
      <div className="glass-container modal-card animate-fade">
        ...
        <form onSubmit={handleSubmit} className="form-auth">
          ...
          <div className="modal-buttons">
            <button type="button" className="btn-link" onClick={fechar}>Cancelar</button>
            <button type="submit" className="btn-main">Finalizar Agendamento</button>
          </div>
        </form>
      </div>
    </div>
...
export default function App() {
  const [view, setView] = useState("home"); // home | auth | agenda
      ...
      {view === "home" && (
        <div className="landing-page animate-fade">
          <nav className="navbar">
            <div className="logo">Agenda<span>Pro</span></div>
            <button className="btn-nav" onClick={() => setView("auth")}>Acessar Agenda</button>
          </nav>
          <section className="hero">
            <div className="hero-inner">
              <h1>Fique sempre na régua com a <span className="text-blue">AgendaPro</span></h1>
              <p>A plataforma definitiva para barbearias de alto nível.</p>
              <button className="btn-main" onClick={() => setView("auth")}>Agende seu horário</button>
            </div>
          </section>
        </div>
      )}
        ...
        <div className="auth-view animate-fade">
          <div className="glass-container auth-card">
            ...
            <p className="toggle-auth">
              ...
              <span onClick={() => { setIsRegistering(!isRegistering); setEmail(""); setPassword(""); }}>
                {isRegistering ? " Faça Login" : " Crie uma conta"}
              </span>
            </p>
            <button className="btn-link" onClick={() => setView("home")}>Voltar</button>
          </div>
        </div>
      )}
        ...
        <div className="booking-view animate-fade">
          <div className="glass-container">
            <header className="agenda-header">
              ...
              <button className="btn-close" onClick={() => setView("home")}>Sair</button>
            </header>
            <div className="grid-times">
              {horarios.map(hora => (
                <button key={hora} className="time-btn" onClick={() => setHorarioSelecionado(hora)}>{hora}</button>
              ))}
            </div>
          </div>
        </div>
